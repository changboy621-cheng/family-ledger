...
| 3.2 | 所有 user-facing 錯誤走 `getErrorMessage`:`QuickAdd.tsx:68` 與 Google 登入(加 try/catch + loading) [tdd:skip:small-wiring] | QuickAdd / Google 登入失敗顯示翻譯訊息而非 raw DB error;lint pass | 3.1 | cc:完了 |
| 3.3 | 帳本頁(LedgerPage)改用常駐 `FAB`(帶 ledgerType),與 Dashboard 一致 [tdd:skip:ui-placement] | family/personal 任意捲動位置可見新增鈕;build OK | - | cc:完了 |
| 3.4 | `TransactionForm` modal 加 `role="dialog"`/`aria-modal`、開啟 focus、focus trap、Esc 關閉、點背景關閉(抽出可重用 `Modal` 元件) [tdd:required] | 鍵盤可操作且 Esc/背景可關;新增互動測試;tests pass | - | cc:完了 |
| 3.5 | `index.html` viewport 加 `viewport-fit=cover`,讓既有 `.safe-bottom` safe-area 生效 [tdd:skip:html-meta] | 有瀏海/home indicator 機型底部 nav 不被遮;build OK | - | cc:完了 |
| 3.6 | 離線偵測:`navigator.onLine` 監聽 → 顯示「離線中」橫幅,儲存錯誤改連線提示 [tdd:skip:browser-api-thin] | 離線時顯示橫幅且錯誤文案為連線提示;build OK | - | cc:完了 |
| 3.7 | 新家庭(solo + 無交易)Dashboard 顯示 `FamilyInvite` 卡 + 空清單「新增第一筆」CTA [tdd:skip:ui-empty-state] | 新使用者落地頁有邀請與新增入口;build OK | 3.3 | cc:完了 |
| 3.8 | toast 改為小型 queue/stack,確保 undo(復原)不被後續 toast 蓋掉 [tdd:required] | 連續刪除/其他 toast 不會吃掉復原鈕;新增測試;tests pass | - | cc:完了 |
